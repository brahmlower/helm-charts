...
| database.host | string | `""` | Hostname or address of the external PostgreSQL server. |
| database.port | integer | `5432` | TCP port the external PostgreSQL server listens on. |
| database.user | string | `""` | Username BookOrbit uses to authenticate to PostgreSQL. |
| database.name | string | `""` | Name of the PostgreSQL database BookOrbit connects to. |
...
| ingress.main.className | string | `""` | IngressClass to use. Leave empty to use the cluster's default</br>IngressClass. |
...
| persistence.data.existingClaim | string | `""` | Name of a pre-existing PVC to use instead of letting the chart create one.</br>Leave empty to have the chart provision and manage the PVC. |
| persistence.data.storageClass | string | `""` | StorageClass requested for the PVC. Leave empty to use the cluster's</br>default StorageClass. |
...
| persistence.books.existingClaim | string | `""` | Name of a pre-existing PVC to use instead of letting the chart create one.</br>Leave empty to have the chart provision and manage the PVC. |
| persistence.books.storageClass | string | `""` | StorageClass requested for the PVC. Leave empty to use the cluster's</br>default StorageClass. |
...
| persistence.books.forceRename | string | `""` | Pins the books PVC to a specific name instead of the chart's auto-derived</br>one. See persistence.data.forceRename above for why this exists. |
