...
| gluetun.config.wireguard_private_key | string | `null` | wireguard specific settings |
| gluetun.config.wireguard_private_key_existing_secret | string | `null` | wireguard_private_key_existing_secret names a pre-existing Secret holding</br>the wireguard private key, for use instead of the plaintext</br>wireguard_private_key value above. Takes precedence when set. |
...
| gluetun.config.wireguard_address | string | `null` | The WireGuard interface address(es) (with CIDR suffix) assigned to this client by the VPN provider. |
| gluetun.config.wireguard_mtu | string | `null` | The MTU to use for the WireGuard interface. Leave empty to use Gluetun's default. |
...
| persistence.configs.storageClassName | string | `null` | The StorageClass to use for the configs PersistentVolumeClaim. Leave empty to use the cluster default. |
| persistence.configs.size | string | `"1Gi"` | The requested size of the configs PersistentVolumeClaim. |
| persistence.downloads.storageClassName | string | `null` | The StorageClass to use for the downloads PersistentVolumeClaim. Leave empty to use the cluster default. |
